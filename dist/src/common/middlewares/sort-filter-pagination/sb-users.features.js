...
const usersFeature = () => {
    return async (req, res, next) => {
        try {
            ...
            console.log(page);
            let query = config_1.supabase.from("sb_users").select("*", { count: 'exact' });
            ...
            if (req.query.fields) {
                ...
                ({ data, count, error } = await config_1.supabase.from('sb_users').select(fields, { count: 'exact' }).range(offset, offset + limit - 1));
                if (error) {
                    throw error;
                }
            }
...
exports.usersFeature = usersFeature;
exports.default = exports.usersFeature;
//# sourceMappingURL=sb-users.features.js.map