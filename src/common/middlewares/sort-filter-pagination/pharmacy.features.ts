...
export const pharmacyFeature = () => {
  return async (req: TPaginationRequest, res: TPaginationResponse, next: NextFunction) => {
    ...
    let query = supabase.from("pharmacy").select("*", { count: 'exact' });
    ...
    if (req.query.search) {
      ...
      query = query.or(
        `name.ilike.%${searchText}%,email.ilike.%${searchText}%`
      )
    ...
    if(req.query.fields) {
      ...
      ({ data, count, error } = await supabase.from('pharmacy').select(fields, { count: 'exact' }).range(offset, offset + limit - 1));
      if (error) {
        throw error;
      }    
    }
