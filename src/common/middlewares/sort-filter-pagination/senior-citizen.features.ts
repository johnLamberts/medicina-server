...
export const seniorCitizenFeature = () => {
  return async (req: TPaginationRequest, res: TPaginationResponse, next: NextFunction) => {
    ...
    let query = supabase.from("senior_citizens").select("*", { count: 'exact' });
    ...
    if(req.query.fields) {
      ...
      ({ data, count, error } = await supabase.from('senior_citizens').select(fields, { count: 'exact' }).range(offset, offset + limit - 1));
      if (error) {
        throw error;
      }    
    }
...
export default seniorCitizenFeature;
