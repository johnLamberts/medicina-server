...
export const usersFeature = () => {
  return async (req: TPaginationRequest, res: TPaginationResponse, next: NextFunction) => {
    ...
    let query = supabase.from("sb_users").select("*", { count: 'exact' });
    ...
    if(req.query.fields) {
      ...
      ({ data, count, error } = await supabase.from('sb_users').select(fields, { count: 'exact' }).range(offset, offset + limit - 1));
      if (error) {
        throw error;
      }    
    }
