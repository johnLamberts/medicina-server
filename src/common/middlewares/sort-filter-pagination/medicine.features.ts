...
export const medicineFeature = () => {
  return async (req: TPaginationRequest, res: TPaginationResponse, next: NextFunction) => {
    ...
    if (req.query.search) {
      ...
      query = query.or(
        `name.ilike.%${searchText}%,brandName.ilike.%${searchText}%,genericName.ilike.%${searchText}%,dosageForm.ilike.%${searchText}%`
      )
    ...
    if(req.query.fields) {
      ...
      ({ data, count, error } = await supabase.from('medicine').select(fields, { count: 'exact' }).range(offset, offset + limit - 1));
      if (error) {
        throw error;
      }    
    }
